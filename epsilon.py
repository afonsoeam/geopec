...
/***************************************************************************
 ...
 >>  Método das Áreas (banda epsilon) implementado de acordo com:
...
from .epsilon_dialog import epsilonDialog
import os.path
import sys, os
...
class epsilon:
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('epsilon', message)
    ...
    def loadVectors(self):
        ...
        self.dlg.comboBox_3.addItems(vector_layers)
    ...
    def getVectorLayerLigacao(self):
        """Gets vector layer specified in combo box"""
        layer = None
    ...
    def setVariables(self):
        ...
        self.ligacao = self.getVectorLayerLigacao()
    ...
    def run(self):
        ...
        self.dlg.pushButton_4.clicked.connect(self.saveVector)
        ...
        if result:
            self.setVariables()
            #aplicando merge para unir as 3 layer de entrada

            merge = processing.run("saga:mergevectorlayers", {'INPUT': [self.test,self.ref,self.ligacao],
                                                      'SRCINFO': True,
                                                      'MATCH': True,
                                                      'MERGED': 'memory:merge'})
            #merge = processing.run("native:mergevectorlayers", {'LAYERS': [self.test,self.ref,self.ligacao],'CRS': None,'OUTPUT': 'merge.shp'})
            self.merge = (merge["MERGED"])
            #self.iface.addVectorLayer(merge["MERGED"], str.split(os.path.basename(merge["MERGED"]), ".")[0],"ogr")

            #Aplicando polygonize para calcular a área do polígono formado anteriormente
            polig = processing.run("qgis:polygonize", {'INPUT': self.merge,
                                                       'KEEP_FIELDS': False,
                                                       'OUTPUT': "memory:polig"})
            self.polig = (polig["OUTPUT"])
            #self.iface.addVectorLayer(polig["OUTPUT"], str.split(os.path.basename(polig["OUTPUT"]), ".")[0], "ogr")
            ...
            #self.iface.addVectorLayer(comptest["OUTPUT"], str.split(os.path.basename(comptest["OUTPUT"]), ".")[0],"ogr")

            # Área A resultante de "polig"
            a = processing.run("qgis:fieldcalculator", {'INPUT': self.polig,
                                                         'FIELD_NAME': 'area',
                                                         'FIELD_TYPE': 0,
                                                         'FIELD_LENGTH': 10,
                                                         'FIELD_PRECISION': 4,
                                                         ...
                                                         'OUTPUT': "memory:area"})
            self.a = (a["OUTPUT"])
            #self.iface.addVectorLayer(a["OUTPUT"], str.split(os.path.basename(a["OUTPUT"]), ".")[0],"ogr")

            join = processing.run("qgis:joinbylocationsummary", {'INPUT': self.comptest,
                                                          'JOIN': self.a,
                                                          'PREDICATE':[0],
                                                          'JOIN_FIELDS':[],
                                                          'SUMMARIES':[5],
            ...
            #self.iface.addVectorLayer(join["OUTPUT"], str.split(os.path.basename(join["OUTPUT"]), ".")[0], "ogr")

            # Cálculo do método das areas
            epsilon = processing.run("qgis:fieldcalculator", {'INPUT': self.join,
                                                        'FIELD_NAME': 'Epsilon',
                                                        'FIELD_TYPE': 0,
                                                        'FIELD_LENGTH': 10,
                                                        'FIELD_PRECISION': 4,
                                                        'NEW_FIELD': True,
                                                        'FORMULA':  'area_sum / compteste',
                                                        'OUTPUT': self.saida})
